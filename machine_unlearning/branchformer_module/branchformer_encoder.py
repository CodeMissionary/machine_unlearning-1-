...
from branchformer_module.abs_encoder import AbsEncoder
from branchformer_module.layers.cgmlp import ConvolutionalGatingMLP
from branchformer_module.fastformer import FastSelfAttention
from branchformer_module.nets_utils import make_pad_mask
from branchformer_module.attention import (  # noqa: H301
    LegacyRelPositionMultiHeadedAttention,
    MultiHeadedAttention,
    RelPositionMultiHeadedAttention,
)
from branchformer_module.embedding import (  # noqa: H301
    LegacyRelPositionalEncoding,
    RelPositionalEncoding,
    ScaledPositionalEncoding,
)
from branchformer_module.layer_norm import LayerNorm
from branchformer_module.repeat import repeat
from branchformer_module.subsampling import *
...
class BranchformerEncoderLayer(torch.nn.Module):
    ...
    def forward(self, x_input, mask, cache=None):
        ...
        if self.attn is not None:
            x1 = self.norm_mha(x1)
            #print(self.attn)#RelPositionMultiHeadedAttention
            if isinstance(self.attn, FastSelfAttention):
                x_att = self.attn(x1, mask)
            else:
                #print(f"po{pos_emb.shape}")#[1, 117, 256]
                if pos_emb is not None:
        ...
        if self.cgmlp is not None:
            ...
            x2 = self.dropout(x2)
        #print(f"x1{x1.shape}")
        #print(f"brran {self.merge_method}")
        # Merge two branches
        if self.use_two_branches:
...
class BranchformerEncoder(AbsEncoder):
    ...
    def __init__(
        ...
        merge_method: str = "learned_ave",#####################################concat  learned_ave
    ...
    def forward(
        ...
        if (
            ...
            xs_pad, masks = self.embed(xs_pad)
...
class BranchformerEncoderAdapterLayer(torch.nn.Module):
    """Branchformer encoder layer module.
    ...
    def __init__(
        ...
        if self.use_two_branches:
            ...
            self.merge_proj = torch.nn.Identity()
        self.adapter=torch.nn.Sequential(torch.nn.Linear(size,adapter_hidden_size),torch.nn.ReLU(),torch.nn.Linear(adapter_hidden_size,size))
    def forward(self, x_input, mask, cache=None):
        """Compute encoded features.
...
class ABranchformerEncoder(AbsEncoder):
    """Branchformer encoder module."""
    ...
    def __init__(
        ...
        super().__init__()
        self._output_size = output_size
        self.adapter_size=adapter_hidden_size
        if rel_pos_type == "legacy":
            if pos_enc_layer_type == "rel_pos":
                pos_enc_layer_type = "legacy_rel_pos"
            if attention_layer_type == "rel_selfattn":
        ...
        self.encoders = repeat(
            num_blocks,
            lambda lnum: BranchformerEncoderAdapterLayer(
                output_size,
                adapter_hidden_size,
                encoder_selfattn_layer(*encoder_selfattn_layer_args)
                if use_attn
    ...
    def forward(
        ...
        """
        #masks = (~make_pad_mask(ilens)[:, None, :]).to(xs_pad.device)
        masks = (~make_pad_mask(ilens, xs=None, length_dim=-1, maxlen=801)[:, None, :]).to(xs_pad.device)
        if (
            isinstance(self.embed, Conv2dSubsampling)
            or isinstance(self.embed, Conv2dSubsampling1)
            or isinstance(self.embed, Conv2dSubsampling2)
